...
# Every teddy bear/item in the game with its own unique ID
const COW := 0
const FARMER := 1
const NURSE := 2
const PATIENT := 3
...
# The string version of all the items' names
const name_arr := [
	"Cow Bear",
	"Farmer Bear",
	"Nurse Bear",
...
# The sprite scene for every item
...
# The sprite scene for every item's icon (a representation 
# of the item that takes up less space)
...
# Used for easy access of the item's scenes
...
# Check if a pairing between 2 items is valid
...
# Check if a pair is equal to another pair, ignoring order
