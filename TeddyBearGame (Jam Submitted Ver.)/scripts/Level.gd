...
func _include_types() -> void:
	if include_farmer_cow:
		item_pool.append(bear_type.FARMER)
		item_pool.append(bear_type.COW)
	if include_nurse_patient:
		item_pool.append(bear_type.NURSE)
		item_pool.append(bear_type.PATIENT)
	if include_zoo_keeper_normal:
		item_pool.append(bear_type.ZOO_KEEPER)
		item_pool.append(bear_type.NORMAL)
	if include_knight_king:
		item_pool.append(bear_type.KNIGHT)
		item_pool.append(bear_type.KING)
	if include_cop_criminial:
		item_pool.append(bear_type.COP)
		item_pool.append(bear_type.CRIMINAL)
	if include_groom_bride:
		item_pool.append(bear_type.GROOM)
		item_pool.append(bear_type.BRIDE)
	if include_karen_manager:
		item_pool.append(bear_type.KAREN)
		item_pool.append(bear_type.MANAGER)
	if include_mama_babies:
		item_pool.append(bear_type.MAMA)
		item_pool.append(bear_type.BABIES)
	if include_angel_devil:
		item_pool.append(bear_type.ANGEL)
		item_pool.append(bear_type.DEVIL)
	if include_honey_pot:
		item_pool.append(bear_type.HONEY_POT)
	if include_bomb_bear:
		item_pool.append(bear_type.BOMB)
